...
exports.create = (req, res)=>{
    ...
    user.save(user).then(data=>{
        ...
        res.status(500).send({message: err.message || "Some error occured while adding user"});
    })
...
exports.find = (req, res)=>{
    if(req.query.id){
        ...
        Userdb.find().then(user=>{
            ...
            res.status(500).send({message: err.message || "Error Occurred while retriving user data" });
        });
    }
};
